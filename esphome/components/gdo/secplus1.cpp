...
static const char *const TAG = "gdo.secplus1";

void Secplus1::setup(GDOComponent *gdo, Scheduler *scheduler, InternalGPIOPin *rx_pin, InternalGPIOPin *tx_pin,
...
void Secplus1::loop() {
  ...
  if ((millis() - this->last_tx_) > 200 &&  // don't send twice in a period
...
void Secplus1::dump_config() { ESP_LOGCONFIG(TAG, "  Protocol: SEC+ v1"); }
...
void Secplus1::sync() {
  ...
  this->scheduler_->cancel_timeout(this->gdo_, "wall_panel_emulation");
  this->wall_panel_emulation();

  this->scheduler_->set_timeout(this->gdo_, "", 45000, [=] {
...
void Secplus1::wall_panel_emulation(size_t index) {
  if (this->wall_panel_emulation_state_ == WallPanelEmulationState::WAITING) {
    ...
    this->scheduler_->set_timeout(this->gdo_, "wall_panel_emulation", 2000, [=] { this->wall_panel_emulation(); });
    return;
    ...
    this->scheduler_->set_timeout(this->gdo_, "wall_panel_emulation", 250, [=] { this->wall_panel_emulation(index); });
  }
}
...
void Secplus1::light_action(LightAction action) {
  ...
  if (action == LightAction::TOGGLE || (action == LightAction::ON && this->light_state == LightState::OFF) ||
...
void Secplus1::lock_action(LockAction action) {
  ...
  if (action == LockAction::TOGGLE || (action == LockAction::LOCK && this->lock_state == LockState::UNLOCKED) ||
...
void Secplus1::toggle_light() { this->enqueue_transmit(CommandType::TOGGLE_LIGHT_PRESS); }

void Secplus1::toggle_lock() { this->enqueue_transmit(CommandType::TOGGLE_LOCK_PRESS); }
...
void Secplus1::toggle_door() {
  ...
  if (this->door_state == DoorState::STOPPED || this->door_state == DoorState::OPEN ||
      this->door_state == DoorState::CLOSED) {
    this->door_moving_ = true;
  }
}

Result Secplus1::call(Args args) { return {}; }
...
void Secplus1::handle_command(const RxCommand &cmd) {
  if (cmd.req == CommandType::QUERY_DOOR_STATUS) {
    ...
    if (!this->is_0x37_panel_ && light_state != this->maybe_light_state) {
      ...
      this->gdo_->received(light_state);
    }
    ...
    this->gdo_->received(obstruction_state);
    ...
    this->gdo_->received(ButtonState::PRESSED);
  } else if (cmd.req == CommandType::TOGGLE_DOOR_RELEASE) {
    this->gdo_->received(ButtonState::RELEASED);
  }
}
