...
namespace esphome
{
    namespace gdo
    {
        ...
        static const char *const TAG = "gdo";
        ...
        void GDOComponent::setup()
        {
            ...
            else
            {
                ...
                this->input_obst_pin_->attach_interrupt(GDOStore::isr_obstruction, &this->isr_store_, gpio::INTERRUPT_FALLING_EDGE);
            }
        ...
        void GDOComponent::init_protocol()
        {
        ...
        {
            ESP_LOGCONFIG(TAG, "Setting up GDO...");
        ...
        void GDOComponent::received(const DoorState door_state)
        {
        ...
        void GDOComponent::received(const LearnState learn_state)
        {
        ...
        void GDOComponent::received(const LightState light_state)
        {
            ESP_LOGD(TAG, "Light state=%s", LightState_to_string(light_state));
            this->light_state = light_state;
        }

        void GDOComponent::received(const LockState lock_state)
        {
            ESP_LOGD(TAG, "Lock state=%s", LockState_to_string(lock_state));
            this->lock_state = lock_state;
        }

        void GDOComponent::received(const ObstructionState obstruction_state)
        {
            if (this->obstruction_from_status_)
            {
        ...
        void GDOComponent::received(const MotorState motor_state)
        {
            ESP_LOGD(TAG, "Motor: state=%s", MotorState_to_string(*this->motor_state));
            this->motor_state = motor_state;
        }

        void GDOComponent::received(const ButtonState button_state)
        {
            ESP_LOGD(TAG, "Button state=%s", ButtonState_to_string(*this->button_state));
            this->button_state = button_state;
        }

        void GDOComponent::received(const MotionState motion_state)
        {
        ...
        void GDOComponent::received(const LightAction light_action)
        {
        ...
        void GDOComponent::received(const Openings openings)
        {
        ...
        void GDOComponent::received(const PairedDeviceCount pdc)
        {
        ...
        void GDOComponent::received(const TimeToClose ttc)
        {
            ESP_LOGD(TAG, "Time to close (TTC): %ds", ttc.seconds);
        }

        void GDOComponent::received(const BatteryState battery_state)
        {
            ESP_LOGD(TAG, "Battery state=%s", BatteryState_to_string(battery_state));
        }

        void GDOComponent::schedule_door_position_sync(float update_period)
        {
        ...
        void GDOComponent::door_position_update()
        {
        ...
        void GDOComponent::obstruction_loop()
        {
        ...
        void GDOComponent::query_status()
        {
            this->protocol_->call(QueryStatus{});
        }

        void GDOComponent::query_openings()
        {
            this->protocol_->call(QueryOpenings{});
        }

        void GDOComponent::query_paired_devices()
        {
            this->protocol_->call(QueryPairedDevicesAll{});
        }

        void GDOComponent::query_paired_devices(PairedDevice kind)
        {
            this->protocol_->call(QueryPairedDevices{kind});
        }

        void GDOComponent::clear_paired_devices(PairedDevice kind)
        {
            this->protocol_->call(ClearPairedDevices{kind});
        }

        void GDOComponent::sync()
        {
            this->protocol_->sync();
        }

        void GDOComponent::door_open()
        {
        ...
        void GDOComponent::door_toggle()
        {
            this->door_action(DoorAction::TOGGLE);
        }

        void GDOComponent::door_action(DoorAction action)
        {
            this->protocol_->door_action(action);
        }

        void GDOComponent::door_move_to_position(float position)
        {
            if (*this->door_state == DoorState::OPENING || *this->door_state == DoorState::CLOSING)
            {
                this->door_action(DoorAction::STOP);
        ...
        void GDOComponent::cancel_position_sync_callbacks()
        {
        ...
        // Learn functions
        void GDOComponent::activate_learn()
        {
            this->protocol_->call(ActivateLearn{});
        }

        void GDOComponent::inactivate_learn()
        {
            this->protocol_->call(InactivateLearn{});
        }

        void GDOComponent::subscribe_rolling_code_counter(std::function<void(uint32_t)> &&f)
        {
            ...
            {
                counter.value.rolling_code_counter.value->subscribe([=](uint32_t state)
                                                                    { defer("rolling_code_counter", [=]
                                                                            { f(state); }); });
            }
        }
        void GDOComponent::subscribe_opening_duration(std::function<void(float)> &&f)
        {
            this->opening_duration.subscribe([=](float state)
                                             { defer("opening_duration", [=]
                                                     { f(state); }); });
        }
        void GDOComponent::subscribe_closing_duration(std::function<void(float)> &&f)
        {
            this->closing_duration.subscribe([=](float state)
                                             { defer("closing_duration", [=]
                                                     { f(state); }); });
        }
        void GDOComponent::subscribe_openings(std::function<void(uint16_t)> &&f)
        {
            this->openings.subscribe([=](uint16_t state)
                                     { defer("openings", [=]
                                             { f(state); }); });
        }
        void GDOComponent::subscribe_paired_devices_total(std::function<void(uint16_t)> &&f)
        {
            this->paired_total.subscribe([=](uint16_t state)
                                         { defer("paired_total", [=]
                                                 { f(state); }); });
        }
        void GDOComponent::subscribe_paired_remotes(std::function<void(uint16_t)> &&f)
        {
            this->paired_remotes.subscribe([=](uint16_t state)
                                           { defer("paired_remotes", [=]
                                                   { f(state); }); });
        }
        void GDOComponent::subscribe_paired_keypads(std::function<void(uint16_t)> &&f)
        {
            this->paired_keypads.subscribe([=](uint16_t state)
                                           { defer("paired_keypads", [=]
                                                   { f(state); }); });
        }
        void GDOComponent::subscribe_paired_wall_controls(std::function<void(uint16_t)> &&f)
        {
            this->paired_wall_controls.subscribe([=](uint16_t state)
                                                 { defer("paired_wall_controls", [=]
                                                         { f(state); }); });
        }
        void GDOComponent::subscribe_paired_accessories(std::function<void(uint16_t)> &&f)
        {
            this->paired_accessories.subscribe([=](uint16_t state)
                                               { defer("paired_accessories", [=]
                                                       { f(state); }); });
        }
        void GDOComponent::subscribe_door_state(std::function<void(DoorState, float)> &&f)
        {
            this->door_state.subscribe([=](DoorState state)
                                       { defer("door_state", [=]
                                               { f(state, *this->door_position); }); });
            this->door_position.subscribe([=](float position)
                                          { defer("door_state", [=]
                                                  { f(*this->door_state, position); }); });
        }
        void GDOComponent::subscribe_light_state(std::function<void(LightState)> &&f)
        {
            this->light_state.subscribe([=](LightState state)
                                        { defer("light_state", [=]
                                                { f(state); }); });
        }
        void GDOComponent::subscribe_lock_state(std::function<void(LockState)> &&f)
        {
            this->lock_state.subscribe([=](LockState state)
                                       { defer("lock_state", [=]
                                               { f(state); }); });
        }
        void GDOComponent::subscribe_obstruction_state(std::function<void(ObstructionState)> &&f)
        {
            this->obstruction_state.subscribe([=](ObstructionState state)
                                              { defer("obstruction_state", [=]
                                                      { f(state); }); });
        }
        void GDOComponent::subscribe_motor_state(std::function<void(MotorState)> &&f)
        {
            this->motor_state.subscribe([=](MotorState state)
                                        { defer("motor_state", [=]
                                                { f(state); }); });
        }
        void GDOComponent::subscribe_button_state(std::function<void(ButtonState)> &&f)
        {
            this->button_state.subscribe([=](ButtonState state)
                                         { defer("button_state", [=]
                                                 { f(state); }); });
        }
        void GDOComponent::subscribe_motion_state(std::function<void(MotionState)> &&f)
        {
            this->motion_state.subscribe([=](MotionState state)
                                         { defer("motion_state", [=]
                                                 { f(state); }); });
        }
        void GDOComponent::subscribe_sync_failed(std::function<void(bool)> &&f)
        {
            this->sync_failed.subscribe(std::move(f));
        }
        void GDOComponent::subscribe_learn_state(std::function<void(LearnState)> &&f)
        {
            this->learn_state.subscribe([=](LearnState state)
                                        { defer("learn_state", [=]
                                                { f(state); }); });
        }
