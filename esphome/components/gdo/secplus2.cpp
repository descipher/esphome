...
// since we write the flash at most every every 5 Min.
// Excesive flashing actions will wear out the NVRAM
...
static const char *const TAG = "gdo.secplus2";
RF rf_timer;
RF *rf = &rf_timer;

void Secplus2::setup(GDOComponent *gdo, Scheduler *scheduler, InternalGPIOPin *rx_pin, InternalGPIOPin *tx_pin,
                     InternalGPIOPin *rf_rx_pin, InternalGPIOPin *rf_tx_pin) {
  this->gdo_ = gdo;
  this->scheduler_ = scheduler;
  this->tx_pin_ = tx_pin;
  this->rx_pin_ = rx_pin;
  rf->set_tx_pin(rf_tx_pin);
  rf->set_rx_pin(rf_rx_pin);
  rf->init_rf_timer();
  rf->start_rx_timer();
...
void Secplus2::sync_helper(uint32_t start, uint32_t delay, uint8_t tries) {
  bool synced = true;
  if (*this->gdo_->door_state == DoorState::UNKNOWN) {
    this->query_status();
    synced = false;
  }
  if (*this->gdo_->openings == 0) {
    this->query_openings();
    synced = false;
  }
  if (*this->gdo_->paired_total == PAIRED_DEVICES_UNKNOWN) {
    this->query_paired_devices(PairedDevice::ALL);
    synced = false;
  }
  if (*this->gdo_->paired_remotes == PAIRED_DEVICES_UNKNOWN) {
    this->query_paired_devices(PairedDevice::REMOTE);
    synced = false;
  }
  if (*this->gdo_->paired_keypads == PAIRED_DEVICES_UNKNOWN) {
    this->query_paired_devices(PairedDevice::KEYPAD);
    synced = false;
  }
  if (*this->gdo_->paired_wall_controls == PAIRED_DEVICES_UNKNOWN) {
    this->query_paired_devices(PairedDevice::WALL_CONTROL);
    synced = false;
  }
  if (*this->gdo_->paired_accessories == PAIRED_DEVICES_UNKNOWN) {
  ...
  if (tries == 2 &&
      *this->gdo_->door_state ==
          DoorState::UNKNOWN) {  // made a few attempts and no progress (door state is the first sync request)
  ...
  if (millis() - start > 30000) {
    ...
    this->gdo_->sync_failed = true;
  } else {
    if (tries % 3 == 0) {
      delay *= 1.5;
    }
    this->scheduler_->set_timeout(this->gdo_, "sync", delay, [=]() { this->sync_helper(start, delay, tries + 1); });
  };
}

void Secplus2::sync() {
  this->scheduler_->cancel_timeout(this->gdo_, "sync");
...
void Secplus2::door_command(DoorAction action) {
  this->send_command(Command(CommandType::DOOR_ACTION, static_cast<uint8_t>(action), 1, 1), IncrementRollingCode::NO,
                     [=]() {
                       this->scheduler_->set_timeout(this->gdo_, "", 150, [=] {
...
void Secplus2::query_openings() { this->send_command(CommandType::GET_OPENINGS); }

void Secplus2::query_paired_devices() {
  const auto kinds = {PairedDevice::ALL, PairedDevice::REMOTE, PairedDevice::KEYPAD, PairedDevice::WALL_CONTROL,
                      PairedDevice::ACCESSORY};
  uint32_t timeout = 0;
  for (auto kind : kinds) {
    timeout += 200;
    this->scheduler_->set_timeout(this->gdo_, "", timeout, [=] { this->query_paired_devices(kind); });
  }
}
...
void Secplus2::clear_paired_devices(PairedDevice kind) {
  ...
  if (kind == PairedDevice::ALL) {
    this->scheduler_->set_timeout(this->gdo_, "", 200, [=] {
      this->send_command(Command{CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::REMOTE) - 1});
    });  // wireless
    this->scheduler_->set_timeout(this->gdo_, "", 400, [=] {
      this->send_command(Command{CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::KEYPAD) - 1});
    });  // keypads
    this->scheduler_->set_timeout(this->gdo_, "", 600, [=] {
      this->send_command(
          Command{CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::WALL_CONTROL) - 1});
    });  // wall controls
    this->scheduler_->set_timeout(this->gdo_, "", 800, [=] {
      this->send_command(Command{CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::ACCESSORY) - 1});
    });  // accessories
    this->scheduler_->set_timeout(this->gdo_, "", 1000, [=] { this->query_status(); });
    this->scheduler_->set_timeout(this->gdo_, "", 1200, [=] { this->query_paired_devices(); });
  } else {
    ...
    this->scheduler_->set_timeout(this->gdo_, "", 200, [=] { this->query_status(); });
    this->scheduler_->set_timeout(this->gdo_, "", 400, [=] { this->query_paired_devices(kind); });
  }
}
...
void Secplus2::activate_learn() {
  ...
  this->scheduler_->set_timeout(this->gdo_, "", 150, [=] { this->send_command(Command{CommandType::LEARN, 1}); });
  this->scheduler_->set_timeout(this->gdo_, "", 500, [=] { this->query_status(); });
}

void Secplus2::inactivate_learn() {
  // Send LEARN twice with nibble = 0 to inactivate learn and get status to update switch state
  this->send_command(Command{CommandType::LEARN, 0});
  this->scheduler_->set_timeout(this->gdo_, "", 150, [=] { this->send_command(Command{CommandType::LEARN, 0}); });
  this->scheduler_->set_timeout(this->gdo_, "", 500, [=] { this->query_status(); });
}
...
void Secplus2::print_packet(const char *prefix, const WirePacket &packet) const {
  ESP_LOG2(TAG, "%s: [%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X]",
           prefix, packet[0], packet[1], packet[2], packet[3], packet[4], packet[5], packet[6], packet[7], packet[8],
           packet[9], packet[10], packet[11], packet[12], packet[13], packet[14], packet[15], packet[16], packet[17],
           packet[18]);
}
...
optional<Command> Secplus2::decode_packet(const WirePacket &packet) const {
  ...
  if ((fixed & 0xFFFFFFFF) == this->client_id_) {  // my commands
    ESP_LOG1(TAG, "[%ld] received mine: rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32, millis(), rolling,
             fixed, data);
    return {};
  } else {
    ESP_LOG1(TAG, "[%ld] received rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32, millis(), rolling, fixed,
             data);
  }
  ...
  ESP_LOG1(TAG, "cmd=%03x (%s) byte2=%02x byte1=%02x nibble=%01x", cmd, CommandType_to_string(cmd_type), byte2, byte1,
           nibble);
...
void Secplus2::handle_command(const Command &cmd) {
  ...
  if (cmd.type == CommandType::STATUS) {
    this->gdo_->received(to_DoorState(cmd.nibble, DoorState::UNKNOWN));
    this->gdo_->received(to_LightState((cmd.byte2 >> 1) & 1, LightState::UNKNOWN));
    this->gdo_->received(to_LockState((cmd.byte2 & 1), LockState::UNKNOWN));
    // ESP_LOGD(TAG, "Obstruction: reading from byte2, bit2, status=%d", ((byte2 >> 2) & 1) == 1);
    this->gdo_->received(to_ObstructionState((cmd.byte1 >> 6) & 1, ObstructionState::UNKNOWN));
    this->gdo_->received(to_LearnState((cmd.byte2 >> 5) & 1, LearnState::UNKNOWN));
  } else if (cmd.type == CommandType::LIGHT) {
    this->gdo_->received(to_LightAction(cmd.nibble, LightAction::UNKNOWN));
  } else if (cmd.type == CommandType::MOTOR_ON) {
    this->gdo_->received(MotorState::ON);
  } else if (cmd.type == CommandType::DOOR_ACTION) {
    auto button_state = (cmd.byte1 & 1) == 1 ? ButtonState::PRESSED : ButtonState::RELEASED;
    this->gdo_->received(button_state);
  } else if (cmd.type == CommandType::MOTION) {
    this->gdo_->received(MotionState::DETECTED);
  } else if (cmd.type == CommandType::OPENINGS) {
    this->gdo_->received(Openings{static_cast<uint16_t>((cmd.byte1 << 8) | cmd.byte2), cmd.nibble});
  } else if (cmd.type == CommandType::SET_TTC) {
    this->gdo_->received(TimeToClose{static_cast<uint16_t>((cmd.byte1 << 8) | cmd.byte2)});
    ...
    this->gdo_->received(pdc);
  } else if (cmd.type == CommandType::BATTERY_STATUS) {
    this->gdo_->received(to_BatteryState(cmd.byte1, BatteryState::UNKNOWN));
  }
...
void Secplus2::send_command(Command command, IncrementRollingCode increment) {
  this->encode_rf_packets(command, this->rf_packet1_, this->rf_packet2_);
  this->transmit_rf_packets();
  ESP_LOG1(TAG, "Send command: %s, data: %02X%02X%02X", CommandType_to_string(command.type), command.byte2,
           command.byte1, command.nibble);
...
void Secplus2::encode_packet(Command command, WirePacket &packet) {
  ...
  uint32_t data = (static_cast<uint64_t>(command.byte2) << 24) | (static_cast<uint64_t>(command.byte1) << 16) |
                  (static_cast<uint64_t>(command.nibble) << 8) | (cmd & 0xff);

  ESP_LOG2(TAG, "[%ld] Encode for transmit rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32, millis(),
           *this->rolling_code_counter_, fixed, data);
  encode_wireline(*this->rolling_code_counter_, fixed, data, packet);
}

void Secplus2::encode_rf_packets(Command command, RFPacket &packet1, RFPacket &packet2) {
  auto cmd = static_cast<uint64_t>(command.type);
  uint64_t fixed = ((cmd & ~0xff) << 24) | this->client_id_;
  uint32_t data = (static_cast<uint64_t>(command.byte2) << 24) | (static_cast<uint64_t>(command.byte1) << 16) |
                  (static_cast<uint64_t>(command.nibble) << 8) | (cmd & 0xff);
  ...
  ESP_LOG2(TAG, "[%ld] Encode for transmit rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32, millis(),
           *this->rolling_code_counter_, fixed, data);
  encode_v2(*this->rolling_code_counter_, fixed, data, 0, packet1, packet2);
}

bool Secplus2::transmit_rf_packets() {
  if (rf_timer.rf_tx_active) {
    return false;
  } else {
    rf_timer.send_rf_cmd(rf_packet1_, rf_packet2_, PACKET_SIZE_BITS, RF_PACKET_LENGTH);
    return true;
  }
}
...
bool Secplus2::transmit_packet() {
  ...
  if (!rf_timer.rf_tx_active) rf_timer.send_rf_cmd(rf_packet1_,rf_packet2_, PACKET_SIZE_BITS, RF_PACKET_LENGTH);
...
void Secplus2::set_client_id(uint64_t client_id) { this->client_id_ = client_id & 0xFFFFFFFF; }
