...
namespace gdo {
    namespace secplus2 {
        ...
        void Secplus2::sync_helper(uint32_t start, uint32_t delay, uint8_t tries)
        {
            ...
            if (tries == 2 && *this->gdo_->door_state == DoorState::UNKNOWN) { // made a few attempts and no progress (door state is the first sync request)
            ...
            if (millis() - start > 30000) {
                ...
                this->scheduler_->set_timeout(this->gdo_, "sync", delay, [=]() {
        ...
        void Secplus2::clear_paired_devices(PairedDevice kind)
        {
            ...
            if (kind == PairedDevice::ALL) {
                this->scheduler_->set_timeout(this->gdo_, "", 200, [=] { this->send_command(Command { CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::REMOTE) - 1 }); }); // wireless
                this->scheduler_->set_timeout(this->gdo_, "", 400, [=] { this->send_command(Command { CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::KEYPAD) - 1 }); }); // keypads
                this->scheduler_->set_timeout(this->gdo_, "", 600, [=] { this->send_command(Command { CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::WALL_CONTROL) - 1 }); }); // wall controls
                this->scheduler_->set_timeout(this->gdo_, "", 800, [=] { this->send_command(Command { CommandType::CLEAR_PAIRED_DEVICES, static_cast<uint8_t>(PairedDevice::ACCESSORY) - 1 }); }); // accessories
